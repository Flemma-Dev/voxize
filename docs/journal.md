...
### 2026-04-22 — Startup latency: audio-first `_initialize` + `WARMING` state

**Problem:** Hotkey → first-real-audio latency was consistently 2+ seconds and occasionally 5–6 seconds. Instrumented the full startup path (new `src/voxize/_trace.py`, opt-in via `VOXIZE_TRACE=1`; pairs with `VOXIZE_TRACE_T0=$EPOCHREALTIME` on the shell side so `t=0` is the actual shell-exec moment, not a Python-internal clock). Two distinct costs fell out of the trace:

1. **openai SDK import (~300–700ms warm, much worse cold).** `from openai import OpenAI` was at module top of `app.py`, and `openai_patches.install()` pulled the same pydantic-model soup at module top of `__main__.py`. Both ran before the window was even constructed, let alone before `audio.start()` opened the mic.
2. **Bluetooth A2DP → HFP switch (~500–1500ms, stack-dependent).** When a BT headset is on A2DP and we open a capture stream, PipeWire/WirePlumber has to renegotiate the profile. Sounddevice callbacks fire during that window but carry silent zeros (or nothing at all). Previously the UI showed "RECORDING" the moment `stream.start()` returned; the user would speak and have the first ~1s of audio vaporise at the OS level.

**Change:** two independent fixes, landed together.

- **Lazy openai imports + audio-first `_initialize`.** Dropped module-level `httpx`, `openai.OpenAI`, and `voxize.cleanup as cleanup_mod` from `app.py`, and the module-level `openai_patches.install()` from `__main__.py`. `_initialize` now: lock → session_dir → api_key → `audio.start()` → `WARMING`, then spawns a daemon `voxize-bootstrap` thread that imports `openai`/`httpx`, runs `openai_patches.install()`, builds the `OpenAI` client, constructs `RealtimeTranscription`, swaps the audio callback (new `AudioCapture.set_on_chunk`, a plain attribute assignment — atomic under the GIL), and calls `transcription.start()`. The WAV is already being written from the moment `audio.start()` returned, so the authoritative batch transcript path never loses bytes regardless of bootstrap timing. `_stop_and_batch` waits on a `threading.Event` (`_bootstrap_done`, 10s timeout) before dispatching batch so `self._client` is guaranteed populated. `_bootstrap_cancelled` short-circuits the thread at every checkpoint so Escape-during-startup doesn't leave a half-wired client behind. The result: `audio.start() returned` lands ~300ms earlier on warm cache; the openai import now overlaps with the user seeing/hearing the mic being "live" instead of blocking it.

- **`WARMING` state.** New state between `INITIALIZING` and `RECORDING`. `_initialize` ends in `WARMING`, and `_check_warming` polls `LevelMeter.level_dbfs` every 40ms. Two consecutive 40ms blocks above `−70 dBFS` → transition to `RECORDING`; hard fallback at 2000ms for users in dead-silent rooms (the mic probably is real, we just can't prove it by RMS). UI for `WARMING` reuses the recording layout with an amber status dot and "Warming up…" label so the user visibly sees they shouldn't speak yet. The real payoff is the case observed in production: one BT-headset run showed `warming: audio detected at 1123ms` — 1.1 seconds of A2DP→HFP renegotiation that previously would have swallowed the user's first sentence. Now the UI says "Recording" only after PCM is actually flowing.

**Transitions added:** `INITIALIZING → WARMING`, `WARMING → RECORDING | TRANSCRIBING | CANCELLED | ERROR`. Stop during `WARMING` goes straight to `TRANSCRIBING` (allowed — batch still has a WAV, even if mostly silent; won't wedge the UI). Escape during `WARMING` → `CANCELLED`. `_on_state_change` keeps the meter/pulse/timer running across `WARMING → RECORDING` so the UI doesn't flicker.

**Mock path:** `_MOCK` mode goes `INITIALIZING → WARMING → RECORDING` immediately via a `GLib.idle_add` shunt (`_mock_warming_to_recording`). No real mic, no real detection — existing mock tests behave as before.

**Ducking moved:** starts on entering `WARMING` (not `RECORDING`) so the BT stack isn't fighting music playback during the profile switch. Restores on leaving the capture phase entirely (WARMING/RECORDING → anything else).

**Warmup ping idempotency:** `_on_state_change(RECORDING)` and the new `_kick_warmup_after_bootstrap` (posted from the bootstrap thread on completion) can both call `_start_warmup`, depending on which lands first — mic-warmed or client-built. Added a guard (`if self._warmup_timer_id is not None: return`) so we don't double-schedule.

**What did not change:** audio capture starts as early as it did before; the WAV is still the safety net (`audio.py` unchanged modulo the `on_chunk=None` + `set_on_chunk()` addition). Live-preview WS can miss early chunks or not connect at all — the user explicitly accepted that trade during brainstorming; batch transcribes the full WAV and is authoritative. No changes to `batch.py` / `cleanup.py` / `transcribe.py`.
...
### 2026-05-22 — Bucketed session retention

**Problem:** the meeting recorder landed reusing the dictation app's single `[storage]` block — 500 newest AND drop > 14 days, applied to every directory under `~/.local/state/voxize/` regardless of suffix. That's a hidden quota collision: a heavy dictation week (200+ overlays) would push older meeting recordings toward the count limit, and a binge of meeting recordings would crowd dictation. The two flavors share zero overlap in what the user wants to keep around — short throwaway dictations vs. multi-hour archives — so a shared quota is wrong by construction.

**Shape:** sessions belong to **buckets** identified by the trailing `-<tag>` on the directory name. No tag → `default` (dictation). `-meeting` → `meeting`. Each app prunes only its own bucket at close time; the dictation app never touches meeting directories, and vice versa. Net effect: 500/14 per bucket, not 500/14 in aggregate. The directory tree stays flat — bucket is a virtual partition derived from the name, not a sub-tree — so a simple `ls ~/.local/state/voxize/` still shows everything chronologically.

**Schema:** TOML subtables. `[storage]` sets the cross-bucket defaults; `[storage.<bucket>]` overrides them for a single bucket. Missing keys inside a subtable inherit from the top-level, so a partial override (`max_age_days = 90` alone) is a true override. Anything dict-valued under `[storage]` is treated as a bucket override; scalar keys are the defaults. Existing `voxize.toml` files keep working unchanged — the top-level keys still drive every bucket until a subtable is added.

**Bucket detection (`storage._bucket_for_name`):** 19-char timestamp + no suffix → `default`; 19-char timestamp + `-X` → bucket `X`; anything else → `None` (stray). Strays are never pruned by any bucket and are logged at INFO once per call — the directory is self-managed in normal use, so a stray means somebody hand-edited a name or dropped an unrelated dir there, and silent eviction would be the wrong move. The `_TIMESTAMP_LEN = 19` magic number is named now; previously it was an inline `name[:19]` in two places.

**API:** `prune_sessions(bucket=BUCKET_DEFAULT)` — dictation calls it bare, the meeting app passes `"meeting"`. Internally it scans the entire state dir once, partitions by `_bucket_for_name`, then applies the existing count + age rules to the in-bucket subset only. `config.CONFIG.storage.for_bucket(name)` returns the resolved `BucketStorageConfig` (a small frozen dataclass with the two int fields) — top-level if no override, override if one exists. `BUCKET_DEFAULT = "default"` is exposed from `storage.py` for callers that want to be explicit.

**The "you might never prune the other bucket" trade-off:** explicit per-bucket scoping means if you only ever launch dictation, your meeting bucket grows without bound, and vice versa. Considered making prune cross-bucket (touch every bucket on every app close) but rejected: the bucket name in `[storage.<bucket>]` is the user's signal of intent for that bucket, and applying it from a different app's close path muddles ownership — the meeting app's lifecycle shouldn't be making decisions about dictation retention. Each app owns its own quota; if both are used, both run their own prune. Acceptable cost for clean ownership.

**Config parser refactor:** factored the inline `max(0, int(val))` int-validation logic out of `_parse()` into `_parse_nonneg_int(raw, key, default, *, path)` so the top-level storage parse and the per-bucket override parse share the same validation path (and the log message keeps the dotted key path — `storage.max_sessions` vs `storage.meeting.max_sessions`). Behavior for the existing top-level keys is byte-identical: same default fallback on missing/non-int/bool, same `max(0, int(val))` clamp on negatives. The `ducking` and `ui` sections still use their inline validators — no scope creep into orthogonal code.

**No migration needed:** bucket detection works retroactively on existing on-disk session names. The first close after upgrading triggers a per-bucket prune; whatever was over-quota under the old single-pool rule gets re-evaluated against its actual bucket. In practice the meeting bucket is so small (this codebase is fresh) that the immediate effect is "dictation now has its own 500/14 quota and meetings have their own 500/14 quota" rather than any visible cleanup pass.

**Files changed:** `src/voxize/config.py` (new `BucketStorageConfig`, `StorageConfig.for_bucket`, `_parse_nonneg_int` helper, template + load log updated), `src/voxize/storage.py` (new `BUCKET_DEFAULT`, `_bucket_for_name`, `prune_sessions(bucket)`), `src/voxize/meeting/app.py` (`_BUCKET = "meeting"`, `_SESSION_SUFFIX = f"-{_BUCKET}"`, passes `_BUCKET` to `prune_sessions`). The dictation `app.py` is unchanged — its `prune_sessions()` call now hits the default `BUCKET_DEFAULT` arg.
